...
void IMU::addIMUnoise(MotionData& data)
{
    ...
    // std::normal_distribution<double> noise(0.0, 0);
    ...
    data.imu_gyro_w = data.Rwb * data.imu_gyro;

    Eigen::Vector3d noise_acc(noise(generator_),noise(generator_),noise(generator_));
    Eigen::Matrix3d acc_sqrt_cov = param_.acc_noise_sigma * Eigen::Matrix3d::Identity();
    data.imu_acc = data.imu_acc + acc_sqrt_cov * noise_acc / sqrt( param_.imu_timestep ) + acc_bias_;

    Eigen::Vector3d noise_vel(noise(generator_),noise(generator_),noise(generator_));
    Eigen::Matrix3d vel_sqrt_cov = param_.acc_noise_sigma * Eigen::Matrix3d::Identity();
    data.imu_velocity = data.imu_velocity + vel_sqrt_cov * noise_vel / sqrt( param_.imu_timestep ) + acc_bias_;
